...
class NetworkInterface {
  private:
    ...
    std::unordered_map<uint32_t, EthernetAddress> map_{};

    std::unordered_map<uint32_t, size_t> map_ms_{};

    std::unordered_map<uint32_t, size_t> map_last_time_ms_{};

    struct wait_queue_entity {
        InternetDatagram dgram;
        Address next_hop;
    };

    std::queue<wait_queue_entity> wait_queue_{};
  ...
  public:
    ...
    void send_arp(EthernetAddress dst, uint32_t dst_ip, uint16_t opcode);
};
