...
def hm(array_):
    ...
    tmp_all = [tmp_2_0, tmp_2_1, tmp_2_2, tmp_2_3]

    if (global_page % 2 != 0 and global_ == 0) or (global_page % 2 == 0 and global_ == 1):
        print("[Error]:  global")
        # search 2^0
        # s_2_0 = np.c_[array_[:, 1], array_[:, 3]]
        # s_2_0 = merge(array_[:, 1], array_[:, 3])
        s_2_0 = []
        for item in array_[:, 1]:
            s_2_0.append(item)
        for item in array_[:, 3]:
            s_2_0.append(item)
        g_p_2_0 = sum(s_2_0)
        g_2_0 = s_2_0[0]
    ...
    if tmp_2_0 is None and tmp_2_1 is None and tmp_2_2 is None and tmp_2_3 is None:
        print("[Stat]: Success")
        return True
    else:
