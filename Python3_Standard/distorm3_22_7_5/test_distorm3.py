...
class TestMode64(unittest.TestCase):
    ...
    BasesInfo = [Regs.RAX, Regs.RCX, Regs.RDX, Regs.RBX, Regs.RSP, Regs.RSI, Regs.RDI, Regs.R8, Regs.R9, Regs.R10,
                 Regs.R11, Regs.R12, Regs.R13, Regs.R14, Regs.R15]
    Indices = ["RAX", "RCX", "RDX", "RBX", "RBP", "RSI", "RDI", "R8", "R9", "R10", "R11", "R12", "R13", "R14", "R15"]
    IndicesInfo = [Regs.RAX, Regs.RCX, Regs.RDX, Regs.RBX, Regs.RBP, Regs.RSI, Regs.RDI, Regs.R8, Regs.R9, Regs.R10,
                   Regs.R11, Regs.R12, Regs.R13, Regs.R14, Regs.R15]
...
class TestInstTable(unittest.TestCase):
    ...
    def test_vex(self):
        ...
        I32("vmovsd xmm1, qword [eax]").check_mnemonic(
            "VMOVSD")  # pre encoding: f2 0f 10 - but VEX.vvvv is not encoded!
...
class TestFeatures(unittest.TestCase):
    ...
    def test_eflags_on(self):
        ...
        # INC
        self.assertEqual(a[1].modifiedFlags,
                         distorm3.D_OF | distorm3.D_SF | distorm3.D_ZF | distorm3.D_AF | distorm3.D_PF)
...
class TestAPI(unittest.TestCase):
    def direct_decompose(self, code, codeOffset, dt, features, maxInstructions):
        ...
        codeInfo = distorm3._CodeInfo(distorm3._OffsetType(codeOffset), distorm3._OffsetType(0),
                                      distorm3._OffsetType(0), ctypes.cast(p_code, ctypes.c_char_p), codeLen, dt,
                                      features)
        status = distorm3.internal_decompose(ctypes.byref(codeInfo), ctypes.byref(result), maxInstructions,
                                             ctypes.byref(usedInstructionsCount))
    ...
    def test_bad_features(self):
        s, count, results = self.direct_decompose(b"\x90", 0, distorm3.Decode32Bits,
                                                  distorm3.DF_MAXIMUM_ADDR16 | distorm3.DF_MAXIMUM_ADDR32, 1)
        self.assertEqual(s, distorm3.DECRES_INPUTERR)
    ...
    def test_drop_skip(self):
        s, count, results = self.direct_decompose(b"\x05\x00\x01", 0, distorm3.Decode32Bits, 0,
                                                  1)  # Skips 05, then returns add [ecx], al
        self.assertEqual(s, distorm3.DECRES_MEMORYERR)
        self.assertEqual(count, 1)
    ...
    def test_fc(self):
        ...
        s, count, results = self.direct_decompose(b"\x90\x75\x00", 0, distorm3.Decode32Bits, distorm3.DF_RETURN_FC_ONLY,
                                                  1)
        self.assertEqual(s, distorm3.DECRES_SUCCESS)
        self.assertEqual(count, 1)

    def test_single_step(self):
        s, count, results = self.direct_decompose(b"\x33\xff\xc0", 0, distorm3.Decode32Bits,
                                                  distorm3.DF_SINGLE_BYTE_STEP, 1)
        self.assertEqual(s, distorm3.DECRES_MEMORYERR)
        self.assertEqual(count, 1)
        self.assertEqual(Mnemonics.get(results[0].opcode, ""), "XOR")
        s, count, results = self.direct_decompose(b"\x33\xff\xc0", 0, distorm3.Decode32Bits,
                                                  distorm3.DF_SINGLE_BYTE_STEP, 2)
        self.assertEqual(s, distorm3.DECRES_MEMORYERR)
        self.assertEqual(count, 2)
        self.assertEqual(Mnemonics.get(results[0].opcode, ""), "XOR")
        self.assertEqual(Mnemonics.get(results[1].opcode, ""), "INC")
        s, count, results = self.direct_decompose(b"\x33\xff\xc0", 0, distorm3.Decode32Bits,
                                                  distorm3.DF_SINGLE_BYTE_STEP, 3)
        self.assertEqual(s, distorm3.DECRES_SUCCESS)
        self.assertEqual(count, 3)
        self.assertEqual(Mnemonics.get(results[0].opcode, ""), "XOR")
        self.assertEqual(Mnemonics.get(results[1].opcode, ""), "INC")
    ...
    def test_fc_and_single_step(self):
        s, count, results = self.direct_decompose(b"\x33\xc3", 0, distorm3.Decode32Bits,
                                                  distorm3.DF_SINGLE_BYTE_STEP | distorm3.DF_RETURN_FC_ONLY, 1)
        self.assertEqual(s, distorm3.DECRES_SUCCESS)
        self.assertEqual(count, 1)
    ...
    def test_fc_dropped(self):
        ...
        self.assertEqual(s, distorm3.DECRES_SUCCESS)
        self.assertEqual(count, 0)
        s, count, results = self.direct_decompose(b"\x66\x67\x90\xc3", 0, distorm3.Decode64Bits,
                                                  distorm3.DF_RETURN_FC_ONLY, 1)
        self.assertEqual(s, distorm3.DECRES_SUCCESS)
        self.assertEqual(count, 1)
